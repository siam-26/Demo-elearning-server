...
const registerUser = async (req, res) => {
  try {
    const {
      ...
      whatsappNumber,
      country,
      referenceNo, // বডি থেকে আসা রেফারেন্স নাম্বার
    } = req.body;

    // ১. সব ফিল্ড আছে কিনা চেক (Validation)
    if (!fullName || !email || !password || !whatsappNumber || !country) {
      return res.status(400).json({
        success: false,
    ...
    // ২. ইমেইল অলরেডি আছে কিনা চেক (ইউনিকনেস নিশ্চিত করতে)
    ...
    // যদি সিস্টেমে অন্তত একজন ইউজার থাকে, তবে রেফারেন্স নাম্বার চেক করবে
    if (totalUsers > 0) {
      if (!referenceNo) {
        return res.status(400).json({
          success: false,
          message: "Reference number is required for registration",
        });
      }

      const refUser = await User.findOne({ referenceNo: referenceNo.trim() });
      if (!refUser) {
        return res.status(400).json({
          success: false,
          message: "Invalid Reference Number! No such user found.",
        });
      }
    }
    ...
    // ৫. নতুন ইউজার তৈরি
    // রেফারেন্স নাম্বারটি মডেলে ডিফল্ট ফাংশন দিয়ে তৈরি হবে, তাই এখানে পাঠানোর দরকার নেই
    ...
    res.status(201).json({
      ...
      message:
        totalUsers === 0
          ? "Seed user registered successfully"
          : "User registered successfully",
      data: {
        id: newUser._id,
        fullName: newUser.fullName,
        email: newUser.email,
        myReferenceNo: newUser.referenceNo, // এই আইডি দিয়ে পরবর্তী ইউজাররা জয়েন করবে
      },
    });
  } catch (error) {
